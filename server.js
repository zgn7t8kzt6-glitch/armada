...
// ── Bed turnover board ──────────────────────────────────────────────────────
// A bed flips to "dirty" when a client discharges (auto, via Kipu sync) or by
// hand for a room switch; RTs mark it clean/open. Overdue = still dirty after a
// shift (~8h). Visible to care/ops staff.
...
app.post('/api/beds', requireAuth, (req, res) => {
  ...
  if (db.prepare(`SELECT 1 FROM bed_turnovers WHERE room = ? AND status = 'dirty' LIMIT 1`).get(room))
...
app.post('/api/beds/:id/clean', requireAuth, (req, res) => {
...
app.post('/api/beds/:id/reopen', requireAuth, (req, res) => {
