...
function renderChartExtras(r){
  ...
  host.innerHTML=`
    ...
    <div class="card no-print"><h3 style="margin:0 0 4px">📱 Resident portal</h3>
      <p class="sub sans" style="margin:0 0 6px">Their own page at <strong>${esc(location.origin)}/portal.html</strong> — house rules, days sober, rent balance, meeting-sheet uploads, and requests. Works on their phone or the house tablet.</p>
      ${r.portal_pin?`<div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap"><div style="font-size:26px;font-weight:800;letter-spacing:6px">${esc(r.portal_pin)}</div><span class="hint">their sign-in PIN — theirs alone</span><button class="btn btn-ghost btn-sm sans" onclick="hPortalPin(${r.id})">↻ Reset PIN</button></div>`
        :`<button class="btn btn-gold btn-sm sans" onclick="hPortalPin(${r.id})">Issue a portal PIN</button>`}</div>`;
  $('residentBody').appendChild(host);
}
async function hPortalPin(rid){
  try{ const d=await api(`/housing/residents/${rid}/portal-pin`,{method:'POST'}); alert('Portal PIN for this resident: '+d.pin+'\n\nWrite it on their welcome card — they sign in at '+location.origin+'/portal.html'); openResident(rid); }
  catch(e){ alert(e.message); }
}
