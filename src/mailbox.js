...
export async function pollMailbox({ max = 25 } = {}) {
  ...
  try {
    ...
    const ins = db.prepare(`INSERT OR IGNORE INTO desk_mail (msg_id, received_at, from_name, from_email, subject, preview, web_link, category, reason, action)
      VALUES (?,?,?,?,?,?,?,?,?,?)`);
    let triaged = 0, surfaced = 0, ignored = 0, newest = since, lastErr = null, rateLimited = 0;
    const sleep = (ms) => new Promise((r2) => setTimeout(r2, ms));
    const runTriage = async (args) => {
      try { return await triageEmail(args); }
      catch (e1) {
        // 429 = we're going too fast — wait properly; anything else gets one quick retry.
        const isRate = /429|rate|too many/i.test(String(e1.message || e1));
        await sleep(isRate ? 20000 : 2500);
        try { return await triageEmail(args); }
        catch (e2) {
          lastErr = String(e2.message || e2).slice(0, 200);
          if (/429|rate|too many/i.test(lastErr)) rateLimited++;
          throw e2;
        }
      }
    };
    ...
    if ((triaged >= max && msgs.length > triaged) || rateLimited >= 2) { /* keep cursor; next run resumes */ } else { setState('mail_last_poll', newest); }
