...
async function kipuSync(){ $('kipuResult').textContent='Syncing…'; try{ const r=await api('/kipu/sync',{method:'POST'}); $('kipuResult').textContent=`Synced from Kipu: ${r.activeNow} active patients (${r.created} new, ${r.matched} updated, ${r.deactivated} no longer active). Census returned ${r.total} records.`; }catch(e){ $('kipuResult').innerHTML='<span style="color:var(--danger)">'+esc(e.message)+'</span>'; } }
