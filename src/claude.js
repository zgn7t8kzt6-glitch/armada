...
export async function triageEmail({ from, subject, preview, myEmail, addressing }) {
  const client = await getClient();
  const response = await client.messages.create({
    model: MODEL,
    max_tokens: 300,
    system: G + 'You triage the inbox of the OWNER of Armada Recovery (behavioral-health company: detox, outpatient, sober living). He gets a LOT of email and wants an EMPTY board unless something truly needs him — this is HIS task list, not a feed of company activity. THE ADDRESSING RULE COMES FIRST: (1) ADDRESSING=list means the mail reached him through a team/distribution list (admissions@, compliance@, billing@ …) — it is the TEAM\'s work. IGNORE it, including admissions coordination, daily attendance/census reports, compliance questions between staff, and operational chatter — surface ONLY a true emergency or something only the owner can decide. (2) ADDRESSING=cc — he was copied for awareness: IGNORE unless it is genuinely high-stakes and he personally must act. (3) ADDRESSING=direct — mail sent TO him: apply the normal rules below. IGNORE aggressively regardless of addressing: newsletters, marketing, cold sales/vendor pitches, industry news, automated notifications, receipts and statements he did not ask about, calendar acceptances, out-of-office replies, social/app notifications, system alerts meant for IT, mass announcements. DECISION: he must approve, choose, sign, or authorize (money, hiring, contracts, legal, clinical escalations). FOLLOWUP: a named person is clearly waiting on HIS reply, or he must chase or schedule something only he can. REVIEW is RARE — reserve it for mail he would genuinely regret missing (a regulator, a referral partner, a key employee, a family complaint, money arriving or leaving). Merely-informational mail is IGNORE, not review. Judge only from what is given; do not invent context.',
    output_config: { effort: 'low', format: { type: 'json_schema', schema: MAIL_TRIAGE_SCHEMA } },
    messages: [{ role: 'user', content: `OWNER'S ADDRESS: ${myEmail || '(unknown)'}\nADDRESSING: ${addressing || 'unknown'} (direct = sent To him; cc = copied; list = reached him via a team/distribution list)\nFROM: ${scrub(String(from || '').slice(0, 160))}\nSUBJECT: ${scrub(String(subject || '').slice(0, 250))}\nPREVIEW: ${scrub(String(preview || '').slice(0, 400))}` }],
  });
  if (response.stop_reason === 'refusal') throw new Error('declined');
  const t = response.content.find((b) => b.type === 'text');
