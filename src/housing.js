...
export const FORM_TEMPLATES = [
  // Day-one form: signed at the door so the resident can get to their room fast;
  // the full packet follows within 72 hours. Field type 'p' renders prose,
  // 'initials' is a per-section initial line (leave blank = not consented).
  { type: 'general_consent', name: 'New Intake General Consent (Day One)', cat: 'Admission', orh: '', sign: true, fields: [
    { k: 'intro', t: 'p', l: 'In an effort to streamline the intake process, we want to prioritize you feeling welcomed to the program and get you in and out as fast as possible on day one so that you aren\'t feeling overwhelmed. The sooner you can get into your own personal space and start unwinding, the faster you\'ll be able to feel at home here. And truly, that\'s our goal. The purpose of this document is to give us general consent to operate and keep you safe, while also keeping us connected so we can make progress in helping you get the care you need. When you sign this form, we\'ll be covering a lot of bases to hold us over until you can meet with the staff member who will be responsible for your case from here on out.' },
    { k: 'init_armada', t: 'initials', l: '1. Initialing here gives Hilltop the ability to communicate with Armada Recovery (323 S Main St Unit 301, Akron, OH 44308) about your care, focusing solely on your presence and attendance until a full release can be signed.' },
    { k: 'init_transport', t: 'initials', l: '2. Initialing here gives Hilltop the right to transport you from any of our registered locations to any destination that you give your consent to attend — to and from group, the store, or outside recovery meetings. It also means that if something were to happen during that transport and Hilltop is deemed not at-fault for an accident of any kind, you waive your right to bring forth a lawsuit trying to hold Hilltop civilly responsible. Without initialing this line, you will be unable to ride in Hilltop\'s van until you do, meaning you will be solely responsible for your own transportation and may only utilize insurance rides or certified ride-share companies that complete background checks on their employees.' },
    { k: 'rules', t: 'p', l: 'The basic rules are as follows:\n• Restriction period can range anywhere from 14–30 days depending on your behavior\n• While on restriction, you will not leave the property without a staff member or Hilltop Senior Peer\n• Curfew is 9pm\n• Attendance at 4 Recovery Program meetings per week is mandatory\n• Compliance with your treatment plan is mandatory — that means attendance is mandatory\n• If you\'re sick and unable to attend, you must have a doctor excuse you via urgent care or ER\n• If you don\'t go to mandatory daily activities, you cannot leave the property or attend elective activities\n• You will not use any mood- or mind-altering substances while a resident at Hilltop\n• If you become a safety threat to yourself or others, you will be removed\n• You will treat staff and other residents with the same kindness you wish to receive in return\n• If there is anything you need, you will communicate and ask. Closed mouths don\'t get fed.\n• Any and all other requirements stated by the staff member reviewing this document with you' },
    { k: 'init_rules', t: 'initials', l: '3. Initialing here means you understand that Hilltop is not responsible for any lost/stolen/missing items that you didn\'t ask to have locked up in the office. It means you understand the basic rules of the program above and consent to acting in accordance with Hilltop\'s rules.' },
    { k: 'init_po', t: 'initials', l: '4. Initialing here means Hilltop can contact your probation/parole officer for any updates deemed necessary. (Leave blank if this doesn\'t apply to you.)' },
    { k: 'po_name', t: 'text', l: 'Probation/Parole officer\'s name' },
    { k: 'po_phone', t: 'text', l: 'Probation/Parole officer\'s phone number' },
    { k: 'po_email', t: 'text', l: 'Email address (if applicable)' },
    { k: 'po_county', t: 'text', l: 'County of probation/parole office' },
    { k: 'init_lease', t: 'initials', l: '5. Initialing here means we will be designing a Lease for you around the address listed below. This address will be what you can use for mail/deliveries and what your Lease agreement will be built around when you meet to finish your intake forms.' },
    { k: 'lease_address', t: 'text', l: 'Address (mail, deliveries & the Lease)' },
    { k: 'outro', t: 'p', l: 'Again, the purpose of this document was to streamline our intake process and get you comfortable faster. We want you to feel at home — it\'s important to us to know that you\'re safe and welcome here. We will meet with you within the next 72 hours to finish your intake documents and make sure we develop a recovery plan with you that\'s achievable and focuses on the goals you want to meet. Welcome home, we\'re so glad you\'re here!' },
  ] },
...
export function buildDailyMovement(date, fid = null) {
  ...
  // House-less rows are legacy Akron data (same ruling as hFrag in mountHousing),
  // so Akron's scoped report must include them or it undercounts vs its own HQ.
  let akron = null; try { akron = db.prepare(`SELECT id FROM org_facilities WHERE fkey='hilltop-akron'`).get()?.id ?? null; } catch { /* registry optional */ }
  const nullOk = (col) => (fid != null && +fid === akron) ? ` OR ${col} IS NULL` : '';
  const fw = fid != null ? ` AND facility_id = ${+fid}` : '';
  const rw = fid != null ? ` AND (r.facility_id = ${+fid}${nullOk('r.facility_id')})` : '';
  const hw = (col) => fid != null ? ` AND (${col} IN (SELECT id FROM housing_houses WHERE facility_id = ${+fid})${nullOk(col)})` : '';
...
export function mountHousing(app) {
  ...
  // A WRITE may only stamp a facility the writer can actually see (mirrors
  // facilityAllowed): admins/org-wide anywhere, row-less legacy users anywhere,
  // everyone else only facilities they hold an access row for.
  const facPickForWrite = (req) => {
    const fid = facPick(req);
    if (!fid || fid < 0) return null;
    try {
      if (!db.prepare(`SELECT 1 FROM org_facilities WHERE id=?`).get(fid)) return null;
      const u = req.user || {};
      if (u.role === 'admin' || ['Executive Director', 'Executive Assistant'].includes(u.job_role || '')) return fid;
      if (!db.prepare(`SELECT 1 FROM user_facility_access WHERE user_id=? LIMIT 1`).get(u.id)) return fid;
      return db.prepare(`SELECT 1 FROM user_facility_access WHERE user_id=? AND facility_id=?`).get(u.id, fid) ? fid : null;
    } catch { return null; }
  };
  ...
  app.get('/api/housing/voice', requireAuth, (req, res) => {
    ...
    const surveys = db.prepare(`SELECT id, key, title FROM housing_surveys WHERE active=1 ORDER BY sort, id`).all().map(s => {
      const responses = db.prepare(`SELECT COUNT(*) c FROM housing_survey_responses WHERE survey_id=?${rFrag(req)}`).get(s.id).c;
      const avg = db.prepare(`SELECT AVG(value_num) a FROM housing_survey_answers an JOIN housing_survey_responses r ON r.id=an.response_id WHERE r.survey_id=? AND an.value_num IS NOT NULL${rFrag(req, 'r.resident_id')}`).get(s.id).a;
      return { ...s, responses, avg: avg != null ? +avg.toFixed(1) : null };
    });
    const recentText = db.prepare(`SELECT an.value_text text, r.created FROM housing_survey_answers an JOIN housing_survey_responses r ON r.id=an.response_id WHERE an.value_text IS NOT NULL${rFrag(req, 'r.resident_id')} ORDER BY r.id DESC LIMIT 25`).all();
    res.json({
  ...
  app.post('/api/housing/houses', requireAuth, requireAdmin, (req, res) => {
    ...
    const fid = facPickForWrite(req) || hilltopAkronId();
  ...
  app.post('/api/housing/residents', requireAuth, (req, res) => {
    ...
    const fid = houseFac || facPickForWrite(req) || hilltopAkronId();
  ...
  app.get('/api/housing/staffhub', requireAuth, (req, res) => {
    ...
    const offRestriction = db.prepare(`SELECT resident_id FROM housing_restrictions WHERE status='active'${rFrag(req)}`).all()
  ...
  app.get('/api/housing/farewell', requireAuth, (req, res) => {
    ...
    const recs = db.prepare(`SELECT would_recommend v FROM housing_alumni_checkins WHERE would_recommend IS NOT NULL${rFrag(req)}`).all().map(r => r.v);
    const nps = recs.length ? Math.round((recs.filter(v => v >= 9).length - recs.filter(v => v <= 6).length) / recs.length * 100) : null;
    const soberRows = db.prepare(`SELECT resident_id, MAX(sober) s FROM housing_alumni_checkins WHERE 1=1${rFrag(req)} GROUP BY resident_id`).all();
  ...
  app.get('/api/housing/intake', requireAuth, (req, res) => {
    const residents = db.prepare(`SELECT id,name,house_id,loc,move_in,status FROM housing_residents WHERE status IN ('active','waitlist')${fFrag(req)} ORDER BY move_in DESC`).all();
    const rows = residents.map(r => ({
